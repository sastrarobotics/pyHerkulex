...
def set_position_d(servoid,dvalue):
...
def save_pid_eeprom(servoid):
    """ saves the PID values from RAM to EEPROM

    Args:
        servoid (int): id of the servo
    """
    p = get_position_p(servoid)	
    i = get_position_i(servoid)
    d = get_position_d(servoid)
